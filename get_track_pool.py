...
def main():
    # command line args
    if len(sys.argv) != 2:
        print('usage: pull_data.py "artist"')
        sys.exit(1)
    ...
    # get artist id
    (artist_id, artist_uri) = get_artist_info(artist)

    # get related artists
    related_artists = get_related_artist_ids(artist_uri)

    # get artist tracks
    artist_tracks = get_artist_tracks(artist_id)

    # get audio features for artist tracks
    print("Getting track features for {}".format(artist))
    artist_track_features = get_track_features(artist_tracks)

    # write to csv
    write_to_csv(artist_track_features, related=False)

    # get track features for related artist tracks
    related_track_features = []
    for related_artist in related_artists:
        # get related artist tracks
        related_artist_tracks = get_artist_tracks(related_artist[1])

        # get audio features for related artist tracks
        print("Getting track features for {}".format(related_artist[0]))
        related_track_features.extend(get_track_features(related_artist_tracks))

    # write related track features to csv
    write_to_csv(related_track_features, related=True)
